...
user_input = input("Enter healthy fracs elements separated by space: ").split()

user_input2 = input("Enter unhealthy fracs separated by space: ").split()
user_input3 = input("enter \"fixed\" for 04 and \"rand\" for random sig matrix")

print(f"job {job_id} running: inlogged with {nr_healthy} healthy samples, and {nr_unhealthy} unhealthy samples being generated for {bayes_or_ciber_and_nnls} with the \"healthy\" distribuiton being {user_input}, and the unhealhty being {user_input2}")
...
def create_fixed_matrix():
    ...
    imputed_df_nonNA_sigmatrix03.index = imputed_df_nonNA_sigmatrix04.index
    imputed_df_nonNA_sigmatrix03.columns = imputed_df_nonNA_sigmatrix04.columns
    ...
    imputed_df_nonNA_sigmatrix02.index = imputed_df_nonNA_sigmatrix04.index
    imputed_df_nonNA_sigmatrix02.columns = imputed_df_nonNA_sigmatrix04.columns
...
print(summed_celltype_fractions)
...
def get_rand_healthy_frac():#gives a random combination of that celltype fraction #celltypes of the givennn type, it is already coarse
    ...
    i=0
    for cell_type in dist_lymphocytes.columns:
        result[cell_type] = float(user_input[i])
        i+=1
...
def get_rand_unhealthy_frac(): #same as other funciton just from pandas dataframe, called summed_celltype_fractions, where every column is a sample we want to randomly take the convex combination of
    result =  {}
    if len(user_input2) != len(summed_celltype_fractions.index):
        raise ValueError(
            f"Expected {len(summed_celltype_fractions.index)} values for unhealthy fractions,  (and these are the 5{summed_celltype_fractions.index})"
            f"but got {len(user_input2)}. Please provide the correct number of values."
        )
    i=0
    for cell_type in summed_celltype_fractions.index:
        #only do this if celltype is not "other"
        result[cell_type] = float(user_input2[i])
        i+=1
...
def get_bayes_input(healthy_sample_nr, unhealthy_sample_nr):
    ...
    if user_input3 == "fixed":
        create_fixed_matrix
    else:
        create_rand_matrix()
    randomized_sig_matrix = pd.read_csv(f"imputed_sig_matrix_nonlogged.txt", sep = "\t") #this already is logged according to the modern log rules
    ...
    with open("cell_state_labels.csv", "r") as f:
        cell_state_labels = f.read().splitlines()
