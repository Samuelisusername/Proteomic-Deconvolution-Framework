...
def create_rand_matrix():
    ...
    combined_values_real = ( #change in here what sig matrix is comprised of 
        weights_real[:, 0] * imputed_df_nonNA_sigmatrix02.values +
        weights_real[:, 1] * imputed_df_nonNA_sigmatrix03.values +
        weights_real[:, 2] * imputed_df_nonNA_sigmatrix04.values
    )
    ...
    combined_df_real.to_csv(f"imputed_sig_matrix_real.txt", sep = '\t')
...
def generate_imputed_sample(healthy):
    ...
    for coarse_group, group_percentages in percentages.items():
        ...
        for fine_celltype, fine_in_coarse_percentage in group_percentages.items():
            mask = imputed_df_nonNA.columns.str.contains(fine_celltype) & imputed_df_nonNA.columns.str.contains('_01_')
           ...
           # rand_nrs = np.random.dirichlet(np.ones(3), size=1)[0] #change this back to 4
            fine_type_sample_inlogged = np.zeros((all_imputed_arrays['B.naive'].shape[0]))
            fine_type_sample_inlogged += np.log1p(imputed_df_nonNA.loc[:, mask].iloc[:, 0])
            fine_type_sample_outlogged = np.zeros((all_imputed_arrays['B.naive'].shape[0]))
            fine_type_sample_nonlogged = imputed_df_nonNA.loc[:, mask].iloc[:, 0]
            # for i in range(3):
            ...
            # fine_type_sample+= imputed_df_nonNA.loc[:, imputed_df_nonNA.columns.str.contains(cell_type)]
            #     fine_type_sample_inlogged += rand_nrs[i]*quadruple_df_inlogged.iloc[:,i]
            #     fine_type_sample_nonlogged += rand_nrs[i] *quadruple_df_nonlogged.iloc[:,i]
            #     fine_type_sample_outlogged += rand_nrs[i]* quadruple_df_outlogged.iloc[:, i]
